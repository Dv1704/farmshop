...
const EmailVerificationPage = () => {
	...
	// Replace `useAuthStore` with `useUserStore` and destructure the necessary states
	...
	useEffect(() => {
		if (code.every((digit) => digit !== "")) {
			handleSubmit(); // Directly call the handleSubmit function
		}
	}, [code]);
	...
	return (
		...
		>
			<motion.div
				...
				className="bg-green-100 p-8 w-full max-w-md rounded-2xl shadow-xl"
			>
					...
					<div className="flex justify-between">
						{code.map((digit, index) => (
							<input
								...
								className="w-12 h-12 text-center text-2xl font-bold bg-gray-200 text-black border-2 border-gray-600 rounded-lg focus:border-green-500 focus:outline-none"
							/>
						))}
					</div>
